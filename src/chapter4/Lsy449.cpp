...
void f4_49()
{
	Mat I=imread("/home/lsy/图片/DIP3E_Original_Images_CH04/Fig0419(a)(text_gaps_of_1_and_2_pixels).tif",0);
	 ...
	 complexI = complexI(Rect(0, 0, complexI.cols & -2, complexI.rows & -2));
	    ...
	    for( int i = 0; i < Proc_RE.rows; ++i )
	           for( int j = 0; j < Proc_RE.cols; ++j )
	           {
	        	   ...
	        	   Proc_RE.at<float>(i, j)=Proc_RE.at<float>(i, j)*exp(-d/2/80/80);
	        	   Proc_IM.at<float>(i, j)=Proc_IM.at<float>(i, j)*exp(-d/2/80/80);
	           }
	    ...
	    //将处理后的虚部和实部矩阵合成一个矩阵，并进行中心平移，还原回（0，2π）的图像
	       ...
	       idft(complexI, complexI,  DFT_REAL_OUTPUT);//1
